...
import { d as db, p as prospectContact, a as prospectOffer, b as prospectList, u as user } from "../../../../../../chunks/index2.js";
...
const POST = async ({ locals, params, request }) => {
  ...
  const offers = await db.select().from(prospectOffer).where(eq(prospectOffer.id, contact.offerId)).limit(1);
  const offer = offers[0];
  if (!offer) {
    return json({ error: "Offre introuvable" }, { status: 404 });
  }
  const listResult = await db.select({ pitch: prospectList.pitch }).from(prospectList).where(eq(prospectList.id, offer.listId)).limit(1);
  const pitch = listResult[0]?.pitch || "";
  const userProfile = await db.select({ name: user.name, company: user.company }).from(user).where(eq(user.id, locals.user.id)).limit(1);
